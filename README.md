...
Prowler(CIS 1.4 + ISMS-P)로 AWS 계정을 스캔, 발견 취약점에 대한 Terraform 보안 강화 코드를 자동 생성·적용·재검증하는 End-to-End CI/CD 보안 파이프라인
...
- **데모 인프라**      : 취약 인프라(55 FAIL) → 자동 보안 코드 생성 → PR·Apply → FAIL 0건 검증
